 ...
 */
@Component({
  selector: 'nc-notifier-container',
  template: `
    <nc-notifier
...
export class NcNotifierContainerComponent implements OnDestroy {
   ...
   */
  notifications: Array<NcNotifierRef> = [];

  /**
   * Notifier configuration
   */
  private readonly _config: NcNotifierConfig;
  ...
  constructor(
    ...
    private _notifierQueue: NcNotifierQueue,
    notifier: NcNotifier
  ) {
   ...
   */
  identifyNotification(index: number, notification: NcNotifierRef): string {
   ...
   */
  onNotificationReady(notificationComponent: NcNotifierComponent): void {
    let currentNotification: NcNotifierRef = this.notifications[
   ...
   * @returns Promise, resolved when done
   */
  private _handleShowAction(action: NcNotifierAction): Promise<NcNotifierRef> {
    return new Promise<NcNotifierRef>((resolve, reject) => {
      this._tempPromiseResolver = resolve; // Save the promise resolve function so that it can be called later on by another method
      this.addNotificationToList(new NcNotifierRef(action.payload));
    });
  }
   ...
   */
  private _continueHandleShowAction(notification: NcNotifierRef): void {
    ...
    if (numberOfNotifications === 1) {
      ...
      if (
        ...
        if (numberOfNotifications > this._config.behaviour.stacking) {
          const oldNotifications: Array<NcNotifierRef> = this.notifications.slice(
            1,
   ...
   * @returns Promise, resolved when done
   */
  private _handleHideAction(action: NcNotifierAction): Promise<undefined> {
    return new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
      const stepPromises: Array<Promise<undefined>> = [];

      // Does the notification exist / are there even any notifications? (let's prevent accidential errors)
      const notification: NcNotifierRef | undefined = this.findNotificationById(
        action.payload
      );
   ...
   * @returns Promise, resolved when done
   */
  private _handleHideOldestAction(action: NcNotifierAction): Promise<undefined> {
   ...
   * @returns Promise, resolved when done
   */
  private _handleHideNewestAction(action: NcNotifierAction): Promise<undefined> {
    // Are there any notifications? (prevent accidential errors)
    if (this.notifications.length === 0) {
      return new Promise<undefined>(
        (resolve: (value?: any) => void, reject: () => void) => {
   ...
   * @returns Promise, resolved when done
   */
  private _handleHideAllAction(action: NcNotifierAction): Promise<undefined> {
    return new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
      // Are there any notifications? (prevent accidential errors)
      const numberOfNotifications: number = this.notifications.length;
   ...
   */
  private addNotificationToList(notification: NcNotifierRef): void {
   ...
   */
  private removeNotificationFromList(notification: NcNotifierRef): void {
    this.notifications = this.notifications.filter(
      (item: NcNotifierRef) => item.component !== notification.component
    );
    this._changeDetectorRef.markForCheck(); // Run change detection because the notification list changed
  }
  ...
  ): NcNotifierRef | undefined {
    return this.notifications.find(
      (currentNotification: NcNotifierRef) =>
