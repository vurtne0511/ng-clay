...
import { NcNotifierAnimationData } from './notifier-animation-data';
import { NcNotifierConfig } from './notifier-config';
import { DEFAULT_NOTIFIER_ICONS, NC_NOTIFIER_ICONS, NcNotifierIcons } from './notifier-icons';
...
export class NcNotifierComponent implements AfterViewInit {
	/**
	 ...
	 */
	@Output() ready: EventEmitter<NcNotifierComponent> = new EventEmitter<NcNotifierComponent>();
	 ...
	 */
	readonly config: NcNotifierConfig;
  ...
  get _hasIcon() { return this.notification.type !== 'default'; }
    ...
    notifier: NcNotifier,
		...
		private _notifierTimer: NcNotifierTimer,
    private _notifierAnimation: NcNotifierAnimation,
    @Optional() @Inject(NC_NOTIFIER_ICONS) public icons: NcNotifierIcons
	) {
	...
	show(): Promise<undefined> {
		return new Promise<undefined>((resolve: () => void, reject: () => void) => {
			...
			if (this.config.animations.enabled && this.config.animations.show.speed > 0) {
				// Get animation data
        const animationData: NcNotifierAnimationData =
