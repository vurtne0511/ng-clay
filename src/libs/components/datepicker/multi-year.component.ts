...
@Component({
  ...
  host: {
    'class': 'nc-calendar-multi-year'
  }
})
export class NcCalendarMultiYear<D> implements AfterContentInit, OnDestroy {
  ...
  /** Handles when a new year is selected. */
  _yearSelected(event: NcCalendarUserEvent<number>) {
  ...
  private _createCellForYear(year: number) {
    ...
    return new NcCalendarCell(year, yearName, this._shouldEnableYear(year), cellClasses);
  }
