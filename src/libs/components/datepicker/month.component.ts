...
@Component({
  selector: 'nc-calendar-month',
  templateUrl: 'month.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    'class': 'nc-calendar-month'
  }
})
export class NcCalendarMonth<D> implements AfterContentInit, OnChanges, OnDestroy {
  ...
  @Output() readonly _userSelection: EventEmitter<NcCalendarUserEvent<D | null>> =
      new EventEmitter<NcCalendarUserEvent<D | null>>();

  /** Emits when any date is activated. */
  @Output() readonly activeDateChange: EventEmitter<D> = new EventEmitter<D>();

  @ViewChild(NcCalendarBody) _calendarBody!: NcCalendarBody;

  /** The label for this month (e.g. "January 2017"). */
  _monthLabel!: string;

  /** Grid of calendar cells representing the dates of the month. */
  _weeks!: NcCalendarCell[][];
  ...
  constructor(
    ...
    @Inject(NC_DATE_RANGE_SELECTION_STRATEGY) @Optional()
            private _rangeStrategy?: NcDateRangeSelectionStrategy<D>) {
  ...
  _dateSelected(event: NcCalendarUserEvent<number>) {
  ...
  _previewChanged({event, value: cell}: NcCalendarUserEvent<NcCalendarCell<D> | null>) {
  ...
  private _createWeekCells() {
    ...
    for (let i = 0, cell = this._firstWeekOffset; i < daysInMonth; i++, cell++) {
      ...
      this._weeks[this._weeks.length - 1].push(new NcCalendarCell<D>(i + 1, dateNames[i],
  ...
  private _fillBeforeCells(): void {
    ...
    const dateNames = this._dateAdapter.getDateNames();
    const beforeWeeks: NcCalendarCell[] = [];

    for (let i = daysInBeforeMonth - this._firstWeekOffset; i < daysInBeforeMonth; i++) {
      beforeWeeks.push(new NcCalendarCell(i + 1, dateNames[i], false));
    }
  ...
  private _fillAfterCells(): void {
    ...
    const dateNames = this._dateAdapter.getDateNames();
    const afterWeeks: NcCalendarCell[] = [], afterRows: NcCalendarCell[] = [];

    for (let i = 0; i < DAYS_PER_WEEK - lastRow.length; i++) {
      afterWeeks.push(new NcCalendarCell(i + 1, dateNames[i], false));
    }

    lastRow.push(...afterWeeks);

    for (let i = 0; i < DAYS_PER_WEEK * (DAYS_MAX_ROWS - this._weeks.length); i++) {
      afterRows.push(new NcCalendarCell(afterWeeks.length + i + 1, dateNames[afterWeeks.length + i], false));
    }
