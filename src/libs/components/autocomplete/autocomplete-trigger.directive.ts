...
import { BOTTOM_LEFT, TOP_LEFT } from '@ng-clay/components/overlay';
...
export const NC_AUTOCOMPLETE_SCROLL_STRATEGY_FACTORY_PROVIDER: Provider = {
  provide: NC_AUTOCOMPLETE_SCROLL_STRATEGY,
  deps: [Overlay],
  useFactory: NC_AUTOCOMPLETE_SCROLL_STRATEGY_FACTORY,
};

export const NC_AUTOCOMPLETE_VALUE_ACCESSOR: Provider = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => NcAutocompleteTriggerDirective),
  multi: true
};
...
@Directive({
  ...
  exportAs: 'ncAutocompleteTrigger',
...
export class NcAutocompleteTriggerDirective implements ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy {
  ...
  constructor(
    ...
    @Inject(NC_AUTOCOMPLETE_SCROLL_STRATEGY) scrollStrategy: any,
  ...
  get panelClosingActions(): Observable<NcOptionSelectionChange | null> {
    return merge(
      ...
      map(event => event instanceof NcOptionSelectionChange ? event : null)
    );
  }

  readonly optionSelections: Observable<NcOptionSelectionChange> = defer(() => {
  ...
  }) as Observable<NcOptionSelectionChange>;

  get activeOption(): NcOptionComponent | null {
  ...
  private _setValueAndClose(event: NcOptionSelectionChange | null): void {
