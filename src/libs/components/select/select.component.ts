...
export class NcSelectChange {
  constructor(public source: NcSelectComponent, public value: any) { }
}
...
export class NcSelectComponent
  ...
  set filter(value: (input: string, option: NcOptionComponent) => boolean) {
    if (typeof value === 'function') {
      this._filter = value;
    } else if (coerceBooleanProperty(value)) {
      this._filter = (keyword: string, option: NcOptionComponent) => {
  ...
  get selected(): NcOptionComponent | NcOptionComponent[] {
  ...
  private _initKeyManager() {
    ...
    this._keyManager = new ActiveDescendantKeyManager<NcOptionComponent>(this.options)
      .withTypeAhead()
  ...
  private _selectValue(value: any, isUserInput = false): NcOptionComponent | undefined {
    const correspondingOption = this.options.find((option: NcOptionComponent) => {
      try {
  ...
  private _propagateChanges(fallbackValue?: any): void {
    ...
    if (this.multiple) {
      valueToEmit = (this.selected as NcOptionComponent[]).map(option => option.value);
    } else {
      valueToEmit = this.selected ? (this.selected as NcOptionComponent).value : fallbackValue;
    }
  ...
  private _clearSelection(skip?: NcOptionComponent) {
  ...
  private _onSelect(option: NcOptionComponent) {
