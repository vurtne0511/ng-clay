...
import { NcTreeFlatDataSource, NcTreeFlattener } from '@ng-clay/components/tree';
...
export class ExampleTreeFlatComponent {
  ...
  treeFlattener = new NcTreeFlattener<ExampleTreeNode, ExampleTreeNode>(
    node => node,
    node => node.level,
    node => node.expandable,
    node => node.children || []
  ...
  dataSource = new NcTreeFlatDataSource(this.treeControl, this.treeFlattener);
